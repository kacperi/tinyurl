...
def test_generate_hash_same_input_same_output(google_url):
...
def test_generate_hash_lengh_always_16(input_url):
    sl = ShortLink(url=input_url)
    assert len(sl.generate_hash()) == 16


def test_generate_hash_returns_correctly_encoded_str(google_url):
...
@pytest.mark.django_db
def test_save_doesnt_override_hash():
    sl = ShortLinkFactory.build()
    initial_hash = "xyz"
    assert initial_hash
...
@pytest.mark.django_db
def test_save_stores_object_correctly():
    sl = ShortLinkFactory.build()
